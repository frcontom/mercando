...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    refreshHandler.value = async () => {
      ...
      if (a) {
        await loadMercadoCategorias(a.id)
        for (const mc of mercadoCategorias.value) await loadProductosByCategoria(mc.id)
      }
    }
  ...
  return (
    <Box sx={{ p: 2 }}>
      {!activo ? (
        <Card><CardContent><Typography color="text.secondary" align="center" sx={{ py: 4 }}>No hay mercado activo. Crea uno desde Mercados.</Typography></CardContent></Card>
      ) : (
        <>
          ...
          <Card
            ...
            sx={{
              ...
              border: '1px solid rgba(144, 202, 249, 0.12)', borderRadius: 2,
              position: 'relative', overflow: 'hidden',
              '&::before': { content: '""', position: 'absolute', top: '-50%', right: '-30%', width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, rgba(144,202,249,0.06) 0%, transparent 70%)' },
              '&:active': { transform: 'scale(0.98)' }, transition: 'transform 0.15s ease',
            }}
          >
            <CardContent sx={{ position: 'relative', zIndex: 1 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.5)', fontWeight: 500, letterSpacing: 1, textTransform: 'uppercase' }}>{activo.nombre}</Typography>
                <Chip label="Activo" size="small" color="success" sx={{ height: 22, '& .MuiChip-label': { px: 1, fontSize: '0.65rem' } }} />
              </Box>
              <Box sx={{ textAlign: 'center', mb: 2 }}>
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.6rem', letterSpacing: 1, textTransform: 'uppercase' }}>Total gastado</Typography>
                <Typography variant="h3" sx={{ fontWeight: 800, color: '#fff', lineHeight: 1.1, mt: 0.3, letterSpacing: -1 }}>{formatCurrency(totalEncontrados)}</Typography>
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.65rem' }}>de {formatCurrency(totalGlobal)} presupuestados</Typography>
              </Box>
              <Box sx={{ mb: 1 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.3 }}>
                  <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.6rem' }}>Progreso</Typography>
                  <Typography variant="caption" sx={{ color: '#69f0ae', fontWeight: 600, fontSize: '0.6rem' }}>{encontradosGlobal}/{todosProductos.length} productos</Typography>
                </Box>
          ...
          <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', display: 'block', mb: 1.5, fontSize: '0.6rem' }}>Categorías</Typography>
          {mercadoCategorias.value.length === 0 ? (
            <Typography variant="body2" color="text.disabled" sx={{ py: 2, textAlign: 'center' }}>Sin categorías asignadas</Typography>
          ) : (
                ...
                return (
                  <Card key={mc.id} onClick={() => navigate(`/mercados/${activo.id}`)} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2, py: 1.5, px: 2, borderRadius: 1, border: '1px solid rgba(105,240,174,0.06)', bgcolor: 'rgba(105,240,174,0.02)', position: 'relative', overflow: 'hidden', '&::before': { content: '""', position: 'absolute', top: '-60%', right: '-15%', width: 100, height: 100, borderRadius: '50%', background: 'radial-gradient(circle, rgba(105,240,174,0.05) 0%, transparent 70%)' }, '&:active': { transform: 'scale(0.98)' }, transition: 'all 0.2s ease' }}>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0, position: 'relative', zIndex: 1 }}>
                      <Typography variant="body2" sx={{ fontWeight: 700, color: 'rgba(255,255,255,0.85)' }}>{mc.categoria?.nombre}</Typography>
                      {prods.length > 0 && (
                    ...
                    <Box sx={{ textAlign: 'right', position: 'relative', zIndex: 1 }}>
                      <Typography variant="h6" sx={{ fontWeight: 800, color: total > 0 ? '#69f0ae' : 'rgba(255,255,255,0.15)', lineHeight: 1 }}>{total > 0 ? formatCurrency(total) : '$0'}</Typography>
                      <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.55rem' }}>{enc}/{prods.length}</Typography>
                    </Box>
                  </Card>
                )
              })}
          ...
          {/* Timeline */}
          <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', display: 'block', mb: 1.5, fontSize: '0.6rem' }}>Últimos movimientos</Typography>
          {todosProductos.filter(p => p.estado !== 'pendiente').length === 0 ? (
            <Typography variant="body2" color="text.disabled" sx={{ py: 2, textAlign: 'center' }}>Aún no has marcado productos</Typography>
          ) : (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              {todosProductos.filter(p => p.estado !== 'pendiente' && p.created_at).sort((a, b) => new Date(b.created_at ?? 0).getTime() - new Date(a.created_at ?? 0).getTime()).slice(0, 8).map(mp => {
                ...
                return (
                  <Box key={mp.id} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 0.6 }}>
                    <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: esEncontrado ? 'rgba(105,240,174,0.12)' : 'rgba(255,152,0,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>{esEncontrado ? '✅' : '🚫'}</Box>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography variant="body2" sx={{ fontWeight: 500, lineHeight: 1.2 }}>{mp.producto?.nombre}</Typography>
                      <Typography variant="caption" color="text.secondary">{mp.cantidad_encontrada || mp.cantidad} {mp.producto?.unidad} · {time}</Typography>
                    </Box>
                    <Typography variant="body2" sx={{ fontWeight: 700, color: esEncontrado && mp.precio > 0 ? '#69f0ae' : 'text.disabled', flexShrink: 0 }}>{esEncontrado && mp.precio > 0 ? `+${formatCurrency(mp.precio * mp.cantidad)}` : '-'}</Typography>
                  </Box>
                )
              })}
            </Box>
